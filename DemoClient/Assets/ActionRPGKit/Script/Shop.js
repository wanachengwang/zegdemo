...
function ShopBuy(id : int , slot : int , price : int){
	...
	if(shopItem){
	//Buy Usable Item	
		var full : boolean = player.GetComponent(Inventory).AddItem(id , 1);
		if(full){
...
function ShopSell(id : int , slot : int , price : int){
	var dataItem : ItemData = database.GetComponent(ItemData);
	if(itemInven){
	//Sell Usable Item	
			player.GetComponent(Inventory).itemQuantity[slot]--;
