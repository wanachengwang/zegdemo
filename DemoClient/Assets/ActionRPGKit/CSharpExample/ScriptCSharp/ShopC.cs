...
public class ShopC : MonoBehaviour {
	...
	void  ShopBuy ( int id  ,   int slot  ,   int price  ){
		...
		if(shopItem){
			//Buy Usable Item	
			full = player.GetComponent<InventoryC>().AddItem(id , 1);
			if(full){
	...
	void  ShopSell ( int id  ,   int slot  ,   int price  ){
		//ItemData dataItem = database.GetComponent<ItemData>();
		if(itemInven){
			//Sell Usable Item	
			player.GetComponent<InventoryC>().itemQuantity[slot]--;
