	...
	IEnumerator  AttackCombo (){
		if (attackCombo [c]) {
			...
			while(conCombo > 0){
				...
				float wait = mainModel.GetComponent<Animation>()[attackCombo[c].name].length;
	...
	IEnumerator  MagicSkill ( int skillID  ){
		if(skillAnimation[skillID]){
			...
			if(GetComponent<StatusC>().mana > manaCost[skillID] && !GetComponent<StatusC>().silence){
				...
				float wait = mainModel.GetComponent<Animation>()[skillAnimation[skillID].name].length -0.3f;
				yield return new WaitForSeconds(wait);
