...
public class GameEntity : MonoBehaviour 
{
    ...
    public string LabName {
        get {
            return entity_name + "_" + modelScale;
        }
    }
	...
	void OnGUI()
	{
		...
		//计算NPC名称的宽高
		Vector2 nameSize = GUI.skin.label.CalcSize (new GUIContent(LabName));
		
		//设置显示颜色为黄色
		GUI.color  = Color.yellow;
		
		//绘制NPC名称
		GUI.Label(new Rect(uiposition.x - (nameSize.x / 2), uiposition.y - nameSize.y - 5.0f, nameSize.x, nameSize.y), LabName);
