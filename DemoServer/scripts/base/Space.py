...
class Space(KBEngine.Base, GameObject):
	...
	def __init__(self):
		...
		self.ApplyScaleWithType()

	def ApplyScaleWithType(self):
		self.tmpCreateEntityDatas[:] = [(x, y, z, self.spaceUTypeB) for x, y, z, w in self.tmpCreateEntityDatas]
	...
	def spawnOnTimer(self, tid):
		...
		INFO_MSG('Space: spawnOnTimer: Space: %i, ID %i' % (self.spaceUTypeB, datas[0]))
